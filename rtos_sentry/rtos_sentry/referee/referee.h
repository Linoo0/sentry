...
/* 
	...
	ID: 0x0005  Byte:  11   	人工智能挑战赛加成与惩罚数据   **
...
typedef enum
{
	...
	// 0x0301
	LEN_robot_interactive_header_data      = 6+10,
...
{ 
  ...
  uint16_t shooter1_17mm_speed_limit;    //速度上限
...
{ 
	...
	uint16_t shooter_heat0;//1号17mm枪口热量
	uint16_t shooter_heat1;  //2号17mm枪口热量
	uint16_t mobile_shooter_heat2; //42mm枪口热量
...
{ 
	...
	robot_interactive_data_t robot_data;
...
typedef struct judge_info_struct {
	...
	ext_ICRA_buff_debuff_zone_status_t	Game_ICRA_buff;      // 0x005              人工智能挑战赛加成与惩罚区状态
	...
	ext_student_interactive_header_data_t   RobotIntereaction_Data;//0x0301   机器人间通信
	ext_robot_command_t             PTZ_Communication;   //0x303     云台手通信
	ext_interact_id_t								ids;								//与本机交互的机器人id
	uint16_t                        self_client;        //本机客户端
	bool	 		IF_REF_ONL;  //裁判系统是否在线
...
{                          
	...
	uint32_t graphic_tpye:3; //直线  矩形  正圆  椭圆  圆弧  浮点  整型  字符  //图形类型
	uint32_t layer:4;        //图层数，0~9
	uint32_t color:4;        //颜色
	uint32_t start_angle:9;  //空    空    空    空    角度  大小  大小  大小 //起始角度
	uint32_t end_angle:9;    //空    空    空    空          位数  空    长度 //终止角度
	uint32_t width:10;       //线宽
	uint32_t start_x:11;     //起点  起点  圆心  圆心  圆心  起点  起点  起点  //起点x坐标
	uint32_t start_y:11;     //起点y坐标同上
	uint32_t radius:10;      //空    空    半径  空    空    、    、    空   //字体大小或半径
	uint32_t end_x:11;       //终点  对顶  空    半轴  半轴  、    、    空    //终点x坐标
	uint32_t end_y:11;       //                              数    数    空   //终点y坐标
...
/*******************************裁判系统数据调用函数**********************************/
void Judge_IF_REF_ONL(void);                                    //心跳函数，判断裁判系统是否在线，暂时没用

//------------PowerHeatData------------//              包
float REF_Report_CHAS_Power(void);   //4                        //瞬时功率
uint16_t REF_Report_CHAS_PowerBuffer(void);   //2               //60焦耳缓冲能量
uint16_t REF_Report_Shooter_Heat(void);    //2                  //1号17mm枪口热量
uint16_t REF_Report_Shooter_Heat_2(void)  ;
//------------------RFID-------------------------//    包
uint8_t REF_Report_RFID_State(void);                            //rfid
//------------ShootInfo----------------//
float REF_Report_RealShootSpeed(void);//4                       //子弹射速
//------------GameRobotStat------------//              包
uint8_t REF_Report_robot_ID(void); //1                          //机器人ID
uint8_t REF_Report_Shoot_SpeedLimit(void); //1                  //子弹速度上限
uint16_t REF_Report_Shoot_CoolingLimit(void);  //2              //子弹热量上限
uint16_t REF_Report_Shoot_CoolingRate(void);  //2 无            //子弹热量冷却速度 单位 /s
uint16_t REF_Report_CHAS_MaxPower(void) ; //2                   //底盘功率限制上限
uint8_t REF_Report_IF_shooter_output(void);//1                  //shooter口输出
uint8_t REF_Report_GAME_TYPE(void);                             //比赛类型判断
uint16_t REF_Report_HP(void);                                   //当前血量
uint8_t REF_Again(void);                                        //比赛判断
uint16_t REF_Bullet(void);                                       //剩余子弹
uint32_t REF_Event(void);                                       //场地事件（前哨战是否被击毁）
...
void Injury_Judgment(void);//受攻击判断，内含标志位置1
void REF_STATE(void);     //比赛开始标志
#endif
