...
void Motor_ReadData(void) //底盘反馈
{
	...
	{		
		MOTOR_DATA[motor_2].msg_cnt++ <= 50	?	get_moto_offset(&MOTOR_DATA[motor_2]) : GET_MODOR_DATA(&MOTOR_DATA[motor_2],motor_2);MOTOR_DATA[motor_2].msg_cnt=100;
		...
		get_total_angle(&MOTOR_DATA[motor_2]);
		GET_MODOR_DATA(&MOTOR_DATA[motor_2],motor_2);
	}
	
	if(rx_message.StdId == MOTOR_ID_READ3)
	{
		MOTOR_DATA[motor_3].msg_cnt++ <= 50	?	get_moto_offset(&MOTOR_DATA[motor_3]) : GET_MODOR_DATA(&MOTOR_DATA[motor_3],motor_3);MOTOR_DATA[motor_3].msg_cnt=100;
		...
		get_total_angle(&MOTOR_DATA[motor_3]);
		GET_MODOR_DATA(&MOTOR_DATA[motor_3],motor_3);
	}
	
	if(rx_message.StdId == MOTOR_ID_READ4)
	{	
		MOTOR_DATA[motor_4].msg_cnt++ <= 50	?	get_moto_offset(&MOTOR_DATA[motor_4]) : GET_MODOR_DATA(&MOTOR_DATA[motor_4],motor_4);MOTOR_DATA[motor_4].msg_cnt=100;
		...
		get_total_angle(&MOTOR_DATA[motor_4]);
...
void GET_MODOR_DATA(moto_measure_t *ptr,int i)
{
	...
	ptr->angle =  (uint16_t)rx_data1[0] << 8 | rx_data1[1];
	ptr->real_current  = (int16_t)(rx_data1[2] << 8 | rx_data1[3]);
	ptr->speed_rpm = ptr->real_current;	//这里是因为两种电调对应位不一样的信息
	ptr->given_current = (int16_t)(rx_data1[4] << 8 | rx_data1[5])/-5;
	ptr->hall = rx_data1[6];
