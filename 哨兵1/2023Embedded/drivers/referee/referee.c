...
{		
	...
	xCom5DMAInit.DMA_PeripheralBaseAddr  = (uint32_t)&(USART1->DR);
	...
	DMA_Init( DMA2_Stream_RX, &xCom5DMAInit );	
	DMA_Cmd( DMA2_Stream_RX, ENABLE);  //stream2   GameRobotStat.robot_id  GameRobotStat.
}

extern uint32_t Refer_time ;//裁判系统
void USART1_IRQHandler( void )
{
	if(USART_GetITStatus(USART1,USART_IT_IDLE)!=RESET)//检测到空闲线路
	{		
		...
		Usart1_Clean_IDLE_Flag = JUDGE_BUFFER_LEN - DMA_GetCurrDataCounter(DMA2_Stream_RX);
...
//    {
//      if(Infantry == 1)//串口坏了
//        VISION_ReadData(Judge_Buffer);		// 读取视觉数据
//    }
    
      
		memset(Judge_Buffer, 0, 200);
		
		DMA_ClearFlag(DMA2_Stream_RX, DMA_IT_TCIF0);
		DMA2_Stream_RX->NDTR = 200;	
