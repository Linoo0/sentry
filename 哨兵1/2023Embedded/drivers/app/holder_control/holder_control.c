...
void holder_control(void)//陀螺仪云台
{
	...
	{
		h_1=PID_CascadeCalc(&double_pid[holder_Yaw],10.0f,Eular[2],(Gyo[2]/10));
		Holder_Set_Current_yaw(h_1);
	}
	else if(RC_CtrlData.rc.s1==3&&RC_CtrlData.rc.s2==1)
	{
		h_1=PID_CascadeCalc(&double_pid[holder_Yaw],30.0f,Eular[2],(Gyo[2]/10));
    Holder_Set_Current_yaw(h_1);
	}
	else if(RC_CtrlData.rc.s1==2&&RC_CtrlData.rc.s2==1)
	{
		h_1=PID_CascadeCalc(&double_pid[holder_Yaw],50.0f,Eular[2],(Gyo[2]/10));
		Holder_Set_Current_yaw(h_1);
	}
	/*pitch轴模式*/
	if(RC_CtrlData.rc.s1==1&&RC_CtrlData.rc.s2==2)
	{
		h_2=PID_CascadeCalc(&double_pid[holder_Pitch],20.0f,Eular[1],(Gyo[0]/10));
		Holder_Set_Current_pitch(h_2);
	}
	else if(RC_CtrlData.rc.s1==3&&RC_CtrlData.rc.s2==2)
	{
		h_2=PID_CascadeCalc(&double_pid[holder_Pitch],10.0f,Eular[1],(Gyo[0]/10));
		Holder_Set_Current_pitch(h_2);
	}
	else if(RC_CtrlData.rc.s1==2&&RC_CtrlData.rc.s2==2)
	{
		h_2=PID_CascadeCalc(&double_pid[holder_Pitch],0.0f,Eular[1],(Gyo[0]/10));
		Holder_Set_Current_pitch(h_2);
	}
}
#elif holder_mode==2
...
void holder_control(void)
{
	...
	else if(RC_CtrlData.rc.s1==3||RC_CtrlData.rc.s1==2)
	{
		...
		{
			holder_pitch=holder_pitch+0.3f;
		}
